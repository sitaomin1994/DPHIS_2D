...
from Algorithm.QuadTree import QuadTree_uniform
from Algorithm.Baseline import identity_algo


def KDTree_standard(data, epsilon, prt_budget, max_height=16, max_cells = 128,
                    max_points=32, split_method='median', noise_method='leaf', seed=0):
    ...
    kd_tree = KDTree(data, epsilon_prt=epsilon_prt, max_cells = 128,
...
def KDTree_hybrid(data, epsilon, prt_budget, level_switch=3, max_height=16, max_cells = 128,
...
class KDTree:
    ...
    def __init__(self, data, epsilon_prt, split_method='mean', max_cells = 128,
                 max_height: int = 15, max_points: int = 1, H=10, l=1E6, seed=0):
    ...
    def build_tree_helper(self, rect: tuple, height: int):
        ...
        if height <= self.l:
            ...
            split_index = self.calculate_split_point(rect, axis)
    ...
    def calculate_split_point(self, rect, axis):
    ...
    def add_noise(self, epsilon, noise_method='geometric', seed=0):
        """
    ...
    def post_weighted_average(self, node: KDTreeNode):
        ...
        if not node.is_leaf():
            ...
            #alpha = (2*eps1**2)/(2*eps1**2 + eps2**2)
    ...
    def generate_private_data(self):
        ...
        if self.root:
            ...
            while len(q) > 0:
                node = q.popleft()
                if node.is_leaf():
                    data_noisy[node.x0:node.x0 + node.h, node.y0:node.y0 + node.w] = node.count_noisy*1.0 / (
                            node.h * node.w)
                else:
                    for child in node.children:
                        q.append(child)
...
if __name__ == '__main__':
    ...
    # K-D tree
    data_kd_em, kd_em = KDTree_standard(data, epsilon, prt_budget=0.5, max_height=16, max_cells = 1000,
                                        split_method='median', max_points=32, noise_method='geometric')

    data_kd_hybrid, kd_hybrid = KDTree_hybrid(data, epsilon, prt_budget=0.2, level_switch=13, max_height=16,
                                              max_points=32, split_method='median', noise_method='geometric')

    data_quad, quad_tree = QuadTree_uniform(data, epsilon, max_height=8, max_points=32, seed=seed)
    data_idt = identity_algo(data, epsilon, seed)
    ...
    print("Quad: {:.4f}".format(evaluation_mre(data, data_quad, query_list)))
    print('idtL{:.4f}'.format(evaluation_mre(data, data_idt, query_list)))
